...
def new(link=TEST_URL, verbose=True):
    ...
    project = ProjectTracker(fandom_abr='hrpf',
        raw_title="NYT Best Seller List Don't Mean Shit (To Joel Farabee)",
        verbose=verbose)

    # Downloading parent work html
    if not link:
        link = input("link to parent work(s): ")
    HTMLDownloader(verbose=verbose).download_html(link, project.id.folder)
    project.files.update_file_paths()

    # Extracting info from html  # TODO not anymore, to rewrite
    ProjectData(project, mode="extract", verbose=verbose)
...
def post(verbose=True):
    ...
    project = ProjectTracker(fandom_abr='hrpf',
        full_title="NYT Best Seller List Don't Mean Shit (To Joel Farabee)",
        verbose=verbose)
    # TODO not anymore, to rewrite
    work = ProjectData(project, mode="saved", verbose=verbose)
